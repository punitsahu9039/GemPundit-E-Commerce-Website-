...
const YellowSapphire = () => {
  ...
  const loadData = () => {
    axios
      .get("http://localhost:3000/cards?name=yellow sapphire")
      .then((res) => {
        setData(res.data);
        setLoading(false);
      })
  ...
  const cardData = data.map((key) => (
    <Col md={3} sm={6} className="mb-4" key={key.id}>
      <Card className="product-card">
        ...
        <Card.Body>
          ...
          <Button 
            variant="primary" 
            onClick={() => addDataToCart(key.id, key.name, key.code, key.price, key.image, key.qty)}>
            Add To Cart
          </Button>
        </Card.Body>
      </Card>
  ...
  return (
    <Container className="text-center my-5">
      <h1 className="display-4 text-primary mb-4">Yellow Sapphire</h1>
      {loading ? (
