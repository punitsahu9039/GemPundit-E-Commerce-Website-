...
function get_nickx_ParameterByName(name, url = window.location.href){
    name = name.replace(/[\[\]]/g, '\\$&');
...
nquery(document).ready(function() {
	...
	nquery('.product_video_iframe').each(function(index, elem){
		if(nquery(this).attr('video-type')=='youtube'){
			...
			let start = get_nickx_ParameterByName('start',nquery(this).attr('data_src'));
			let autoplay = get_nickx_ParameterByName('autoplay',nquery(this).attr('data_src'));
			let mute = get_nickx_ParameterByName('mute',nquery(this).attr('data_src'));
			let nocookie = '';
			if(yt_youtube_url.search("nocookie") > 0){
				nocookie = '-nocookie';   
			}
		    nquery(this).attr('src','https://www.youtube'+nocookie+'.com/embed/'+iframe_src+'?rel=0&autoplay='+autoplay+'&mute='+mute+'&showinfo=0&enablejsapi=1&start='+start);
	...
	if(nquery('.nickx-slider-for').length > 0) {
		...
		nquery(document).on('found_variation', function(event,variation) {
